...
static uint16_t CIA402_BuildStatusword(void)
{
    ...
    switch (s_state)
    {
        case CIA402_NOT_READY_TO_SWITCH_ON: sw = SW_NOT_READY_TO_SWITCH_ON_VAL; break;
        case CIA402_SWITCH_ON_DISABLED:      sw = SW_SWITCH_ON_DISABLED_VAL;      break;
        case CIA402_READY_TO_SWITCH_ON:      sw = SW_READY_TO_SWITCH_ON_VAL;      break;
        case CIA402_SWITCHED_ON:              sw = SW_SWITCHED_ON_VAL;              break;
        case CIA402_OPERATION_ENABLED:        sw = SW_OPERATION_ENABLED_VAL;        break;
        case CIA402_QUICK_STOP_ACTIVE:        sw = SW_QUICK_STOP_ACTIVE_VAL;        break;
        case CIA402_FAULT_REACTION_ACTIVE:    sw = SW_FAULT_REACTION_ACTIVE_VAL;    break;
        case CIA402_FAULT:                    sw = SW_FAULT_VAL;                    break;
        default:                              sw = SW_FAULT_VAL;                    break;
    }
    sw |= SW_REMOTE;
    if (s_state != CIA402_NOT_READY_TO_SWITCH_ON) sw |= SW_VOLTAGE_ENABLED;
    if (MC_GetCurrentFaultsMotor1() != 0u)        sw |= SW_WARNING;
    
    /* 握手应答 */
    if (s_pp_setpoint_pending) sw |= SW_SETPOINT_ACK;
...
static void CIA402_UpdateActualValues(void)
{
    ...
    if (!s_pos_tracker_ready) return;

    float_t mcsdk_pos_rad = MC_GetCurrentPosition1();
    float_t delta_rad = mcsdk_pos_rad - s_last_mcsdk_pos_rad;
    
    if (delta_rad > CIA402_PI)       delta_rad -= CIA402_2PI;
    else if (delta_rad < -CIA402_PI) delta_rad += CIA402_2PI;
    
    s_continuous_pos_float += (delta_rad * CIA402_POS_SCALE);
    s_last_mcsdk_pos_rad = mcsdk_pos_rad;
    
    while (s_continuous_pos_float >= (float_t)MODULO_RANGE) s_continuous_pos_float -= (float_t)MODULO_RANGE;
    while (s_continuous_pos_float < 0.0f)                  s_continuous_pos_float += (float_t)MODULO_RANGE;
    
    OD_RAM.x6064_positionActualValue = (int32_t)(s_continuous_pos_float + 0.5f);
    if (OD_RAM.x6064_positionActualValue >= MODULO_RANGE) OD_RAM.x6064_positionActualValue = 0;

    float_t nominal = (float_t)NOMINAL_CURRENT_A;
    if (nominal < 0.1f) nominal = 1.0f;
...
static void CIA402_StateMachineProcess(void)
{
    ...
    switch (s_state)
    {
        case CIA402_NOT_READY_TO_SWITCH_ON:
            if (mc_state != ICLWAIT && mc_state != OFFSET_CALIB && mc_state != CHARGE_BOOT_CAP) s_state = CIA402_SWITCH_ON_DISABLED;
            break;
        case CIA402_SWITCH_ON_DISABLED:
            if ((cw & 0x0007) == 0x0006) s_state = CIA402_READY_TO_SWITCH_ON;
            break;
        case CIA402_READY_TO_SWITCH_ON:
            if ((cw & 0x0007) == 0x0007) s_state = CIA402_SWITCHED_ON;
            break;
        case CIA402_SWITCHED_ON:
            if ((cw & 0x000F) == 0x000F)
            {
                if (mc_state == IDLE)
                {
                    if (MC_StartMotor1()) printf("[CIA402] Motor starting...\r\n");
                }
        ...
        case CIA402_OPERATION_ENABLED:
            ...
            else
            {
                ...
                if (OD_RAM.x6060_modesOfOperation == 1)
                {
                    ...
                    if (new_setpoint && !last_setpoint)
                    {
                        ...
                        if (cw & CW_ABS_REL) target_inc = (current_inc + OD_RAM.x607A_targetPosition) % MODULO_RANGE;
                        else target_inc = OD_RAM.x607A_targetPosition % MODULO_RANGE;
                        
                        if (target_inc < 0) target_inc += MODULO_RANGE;
                        
                        int32_t diff_inc = target_inc - current_inc;
                        if (diff_inc > MODULO_HALF)   diff_inc -= MODULO_RANGE;
                        if (diff_inc < -MODULO_HALF)  diff_inc += MODULO_RANGE;
            ...
            break;
        case CIA402_FAULT:
            if (cw & 0x0080) { MC_AcknowledgeFaultMotor1(); s_state = CIA402_SWITCH_ON_DISABLED; }
            break;
        default: break;
    }
}
...
void CIA402_Process(void)
{
    ...
    if (mode == 1) {
        ...
        if (diff > MODULO_HALF) diff -= MODULO_RANGE;
        if (diff < -MODULO_HALF) diff += MODULO_RANGE;
        reached = (abs(diff) <= POS_WINDOW_DEFAULT) && (!s_pp_setpoint_pending);
    }
    if (reached) { if (s_target_reached_timer < WINDOW_TIME_MS) s_target_reached_timer++; else sw |= SW_TARGET_REACHED; }
    else { s_target_reached_timer = 0; }
